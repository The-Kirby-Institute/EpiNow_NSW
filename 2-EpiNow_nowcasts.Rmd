...
startDate <- "2021-09-15" # Filter to after this date (speeds things up) 
...
if (postcodeRun) {
  ...
  # resultsFolder <- "Hunter_New_England"
}
...
if (postcodeRun) {
  ...
  dateCases <- max(cases$date) # Need to check to make sure postcode
                               # file name correct
  ...
  postcodes <- read_csv(paste0("data/postcode_suburb_name-", 
    ...
    mutate(region = as.character(postcode), 
      name = paste0(as.character(postcode), "-", 
      str_replace(name, "/","_")))
